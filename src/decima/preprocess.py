...
def var_to_intervals(ad, chr_end_pad=10000, genome="hg38", seq_len=524288, crop_coords=163840):
    ...
    print(f"The interval size is {seq_len} bases. Of these, {crop_coords} will be upstream of the gene start and {seq_len - crop_coords} will be downstream of the gene start.")
    ...
    ad.var.loc[ad.var.strand == "+", "gene_mask_start"] = ad.var.loc[ad.var.strand == "+", "gene_start"] - ad.var.loc[ad.var.strand == "+", "start"]
    ad.var.loc[ad.var.strand == "-", "gene_mask_start"] = ad.var.loc[ad.var.strand == "-", "end"] - ad.var.loc[ad.var.strand == "-", "gene_end"]
...
def aggregate_anndata(
    ad,
    by_cols=['cell_type', 'tissue', 'organ', 'disease', 'study', 'dataset', 'region', 'subregion', 'celltype_coarse'],
    sum_cols=['n_cells']
):
    # Get column names
