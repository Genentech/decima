...
def _predict_variant_effect(
    ...
    results = model.predict_on_dataset(
        dataset, device=device, batch_size=batch_size, num_workers=num_workers, float_precision=float_precision
    )
...
def predict_variant_effect(
    ...
    if output_pq is not None:
        ...
        _write_vep_warnings(warning_counter, num_variants, output_pq)
    else:
        _df = list()
        for df, warnings, _num_variants in results:
            num_variants += _num_variants
    ...
    _log_vep_warnings(warning_counter, num_variants, genome_path)

    if output_pq is None:
        return pd.concat(_df)


def _log_vep_warnings(warning_counter: Counter, num_variants: int, genome_path: str):
    logger = logging.getLogger("decima")
...
def _write_vep_warnings(warning_counter: Counter, num_variants: int, output_path: str):
    if warning_counter.total():
        with open(Path(output_path).with_suffix(".warnings.log"), "w") as f:
            for warning, count in warning_counter.items():
