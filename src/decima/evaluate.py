...
def compare_marker_zscores(ad, key='cell_type'):
    ...
    marker_df = (marker_df_pred
             .merge(marker_df_obs, on=['gene', key], suffixes=('_pred','_obs'))
             )
    return marker_df
...
def compute_marker_metrics(marker_df, key='cell_type', tp_cutoff=1):
    ...
    for k in set(marker_df[key]):
        ...
        # compute corrs
        corr = pearsonr(
            curr_marker_df['score_obs'], curr_marker_df['score_pred']
        )[0]
        ...
        # append df
        curr_df = pd.DataFrame({key:[k], 'n_positive':[n_positive], 'pearson':[corr], 'auprc':[auprc], 'auroc':[auroc]})
